...
class GitHubHandler {
  ...
  _createWebhookHandler() {
    ...
    handler.on('push', this.pushRequestHandler.bind(this));
  ...
  pullRequestHandler(event, cb) {
    ...
    this.processWebhookEvent(request, (error, build) => {
      this.logger.info({type: request.type, slug: request.slug, err: error}, 'Pull request processed');
  ...
  processWebhookEvent(request, cb) {
    ...
    this.api.findProjectByRepo(request, (error, project) => {
      if (error || !project) {
        this.logger.info({error}, `Project for github repo ${request.slug} not found`);

        cb(error || new Error('Project not found'));
      }
