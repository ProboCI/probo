...
GithubHandler.prototype.processRequest = function(request, done) {
  var self = this;
  self.log.info({type: request.type, id: request.id}, `Processing request for ${request.slug} on commit ${request.payload.pull_request.sha}`);
  ...
  this.api.findProjectByRepo(request, function(error, project) {
    ...
    self.log.info(`Found project for PR ${project.id}`);
