 ...
 */
GithubHandler.prototype.start = function() {
  var self = this;
  this.server.listen(self.options.port, function() {
    self.log.info('Now listening on port ' + self.options.port);
  });
};
...
GithubHandler.prototype.pullRequestHandler = function(event) {
  ...
  self.fetchProvisoYamlConfigFromGithub(pullRequest, function(error, data) {
    var statusInfo = {
      state: 'pending',
      description: 'Your QA environment is being provisioned...',
      context: 'Proviso QA',
    }
    self.postStatusToGithub(pullRequest, statusInfo, function(error) {
      if (error) {
