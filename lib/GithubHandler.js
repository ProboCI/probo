...
GithubHandler.prototype.getProjectInfo = function(request, cb){
  if(this.options.api.url){
    return this.api.findProjectByRepo(request.service, request.slug, request.host, cb);
  } else {
    setImmediate(function(){
      cb(null, {
        ...
        repo: request.repo,
        slug: request.slug,
        service_auth: {token: this.options.githubAPIToken, type: 'token'}
      });
    });
  }
}
...
GithubHandler.prototype.processRequest = function(request, cb){
  ...
  this.getProjectInfo(request, function(error, project){
    if(error || !project){
    ...
    self.fetchProvisoYamlConfigFromGithub(project, request.sha, function(error, config) {
      ...
      self.api.submitBuild(build, project, function(err, build){
        ...
        // pretting environment
        self.api.setBuildStatus(build.id, "ci/tests", {
          state: "pending",
          description: "Waiting on environment " + new Date()
        });
        self.api.setBuildStatus(build.id, "ci/env", {
          state: "pending",
          description: "Building environment " + new Date()
             ...
             setTimeout(function(){
               self.api.setBuildStatus(build.id, "ci/env", {
                 state: "success",
                 description: "Environment built " + new Date()
               });

               self.api.setBuildStatus(build.id, "ci/tests", {
                 state: "pending",
                 description: "Running tests " + new Date()
               }, function response(error, status){

                 setTimeout(function(){
                   self.api.setBuildStatus(build.id, "ci/tests", {
                     state: "success",
                     description: "Tests passed " + new Date()
                   }, function response(error, status){});
                 }, 2000);
