...
const dockerDecoder = require('docker-raw-stream');
...
const _stdout = Symbol('_stdout');
const _stderr = Symbol('_stderr');
...
class ContainerExec extends events.EventEmitter {
  ...
  constructor(container, command, options) {
    ...
    this[_stdout] = new MagicThrough();
    this[_stderr] = new MagicThrough();
    this[_stdin] = new MagicThrough();
  ...
  get containerIdShort() {
    return this.container.containerId.substr(0, 12);
  }
  ...
  create(done) {
    ...
    self[_container].dockerContainer.exec(self.createOptions(), function(error, exec) {
      if (error) return self.errorHandler(error, done);
      self.exec = exec;
      self.log.debug(`Exec ${this.id} created on container ${this.containerIdShort}`);
      if (error) {
        if (error.message === 'write EPIPE') {
          return self.retryRun(error, done);
        }
        return self.errorHandler(error, `${self.id} on container ${this.containerIdShort}: Failed to exec command \`${error.message}\``, done);
      }
      done();
    });
  }
  ...
  start(done) {
    var self = this;
    self.log.debug(`starting exec ${this.id} on ${this.containerIdShort}`);
    self.exec.start({stream: true, stdin: true, stdout: true}, function(error, stream) {
      self.log.debug(`exec ${this.id} running on container ${this.containerIdShort}`);
  ...
  inspect(done) {
    // Done is required, if done is not provided we will presume that this is an
    // inspect invocation invoked by node.js `util.inspect()` called by
    // `console.log()`.
    if (!done) {
      return this;
    }
    ...
    self.exec.inspect(function(error, data) {
      ...
      self.log.debug(`exec ${self.exec.id.substr(0, 12)} on container ${self.containerIdShort} completed with ${self.exitCode}`);
