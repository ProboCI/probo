...
class ContainerExec {
  ...
  constructor(container, command, options) {
    ...
    this.log = options.log || logger.createLogger();
  ...
  set exec(exec) {
    this.log = this.log.child({execId: exec.Id});
    this[_exec] = exec;
  }
  ...
  run(done) {
    ...
    self.create(function(error) {
      ...
      self.start(function(error) {
        if (error) return self.errorHandler(error, done);
        done(null, {ExitCode: 0});
        /*
        self.log.info('Logging output…');
        self.inspect(function(error, data) {
          done(error, data);
        });
        */
      });
  ...
  create(done) {
    ...
    self[_container].dockerContainer.exec(/* self.createOptions() */ options, function(error, exec) {
  ...
  start(done) {
    var self = this;
    self.log.info(self.logContext, 'starting the exec');
    self.exec.start({stream: true, stdin: true, stdout: true}, function(error, stream) {
      if (error) return this.errorHandler(error, done);
      self[_container].dockerContainer.modem.demuxStream(stream, self.stdoutStream, self.stderrStream);
      stream.on('end', done);
    });
  }
  ...
  inspect(done) {
    ...
    self.exec.inspect(function(error, data) {
      ...
      // TODO: Tidy up the data that's returned here and set some properties on this exec.
      self.log.info('exec completed with', data.ExitCode);
