 ...
 *
 * @param {Function} done - callback that takes err and an object as arguments. The object has {virtual, real}, for the size of the inderlying image and container layer respectively
 */
Container.prototype.getDiskUsage = Promise.promisify(function(done) {
  var result = {real: undefined, virtual: undefined};
  ...
  this.container.inspect({size: true}, function(err, info) {
    ...
    result.real = (info.SizeRw || null);
    result.virtual = (info.SizeRootFs || null);
    done(null, result);
  });
});
