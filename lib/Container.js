...
const Solr = require('./plugins/TaskRunner/Solr');
const Database = require('./plugins/TaskRunner/Database');
const NodeJS = require('./plugins/TaskRunner/NodeJS');
...
Container.prototype.buildSetupTasks = Promise.promisify(function (config, done) {
  var jobConfig = this.jobConfig;

  this.build.config.global = config;
  ...
  if (jobConfig.assets && this.assetsOpts) {
    this.assetsOpts.bucket =
      (project.assets && project.assets.bucket) || project.id;
    this.assetsOpts.assets = jobConfig.assets;
    var ad = new AssetDownloader(container, this.assetsOpts);
    tasks.push(ad);
  }

  // 3. Handle Database if configured and not using the image: yaml
  if (this.database) {
    var db = new Database(container, build.config);
    tasks.push(db);
  }

  // 4. Handle Solr if configured and not using the image: yaml
  if (this.solr) {
    var solr = new Solr(container, build.config);
    tasks.push(solr);
  }

  // 5. Handle installing NodeJS if configured.
  if (this.nodejs) {
    const nodejs = new NodeJS(container, build.config);
    tasks.push(nodejs);
  }
...
Container.prototype.create = Promise.promisify(function (done) {
  ...
  const extraHosts = this.jobConfig.extraHosts ? this.jobConfig.extraHosts : [];

  // By default, use 2.5 CPU per build. Changeable in .probo.yaml
  const cpuPeriod = this.jobConfig.cpuPeriod ? this.jobConfig.cpuPeriod : 100000;
  const cpuQuota = this.jobConfig.cpuQuota ? this.jobConfig.cpuQuota : 250000;
