...
class Container extends EventEmitter {
  ...
  constructor(options) {
    super();
    options = options || {};
    this[_dockerClient] = new Docker(options.dockerConnectionInfo);
    ...
    if (this.containerId) {
      this.state = 'unknown';
      this.dockerContainer = this.dockerClient.getContainer(this.containerId);
    }
  ...
  create(done) {
    ...
    createOptions.HostConfig.PortBindings = commandInfo.portBindings;
    createOptions.HostConfig.Binds = self.binds;
    ...
    docker.createContainer(createOptions, function(error, container) {
      self.log.info(`container ${container.id} created`);
  ...
  inspect(done) {
    ...
    this.dockerContainer.inspect(function(error, info) {
  ...
  /**
   * @param {String} before - The event to emit immediately.
   * @param {String} after - The event to emit after the function is successfully called.
   * @param {Function} done - The callback to call upon completion.
  ...
  getDiskUsage(done) {
    ...
    this.dockerContainer.inspect({size: true}, function(error, info) {
      if (error) {
