...
Container.prototype.getDiskUsage = Promise.promisify(function(done){
  if(process.getuid() !== 0){
    return done(new Error("getDiskUsage must be run as root"))
  }
  ...
  this.container.inspect(function(err, info){
    ...
    if(info.Driver != "aufs"){
      return done(new Error(`Container Driver must be 'aufs', found '${info.Driver}'`))
    }
...
function wrapCallbackWithEmitter(emitter, event_name, callback){
  return function(){
    emitter.emit.apply(emitter, Array.prototype.slice.call(arguments).unshift(event_name))
