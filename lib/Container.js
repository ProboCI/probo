 ...
 *
 * @param {Function} done - callback that takes err and an object as arguments. The object has {virtualSize, realSize}, for the size of the inderlying image and container layer respectively
 */
Container.prototype.getDiskUsage = Promise.promisify(function(done) {
  var result = {realSize: undefined, virtualSize: undefined};
  ...
  this.container.inspect({size: true}, function(err, info) {
    ...
    result.realSize = (info.SizeRw || null);
    result.virtualSize = (info.SizeRootFs || null);
    done(null, result);
  });
});
