...
class Container extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.dockerContainer = this.containerId ? this.docker.getContainer(this.containerId) : null;
  ...
  create(done) {
    ...
    docker.createContainer(createOptions, function(error, container) {
      ...
      if (error) return done(error);
      self.log.info({containerId: container.id}, 'container created');
      self.log.info({containerId: container.id}, 'starting container');
      if (self.attachLogs) {
  ...
  remove(done) {
    this.dockerContainer.remove(done);
  }
  ...
  stop(done) {
    this.dockerContainer.stop(done);
  }

  restart(done) {
    this.dockerContainer.restart(done);
  }

  start(done) {
    this.dockerContainer.start(done);
  }
