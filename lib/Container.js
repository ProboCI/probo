...
Container.prototype.buildTasks = Promise.promisify(function(done) {
  var self = this;
  var config = this.jobConfig;
  var tasks = [];

  var steps = config.steps || {provisioner: config.provisioner}
  steps = Array.isArray(steps) ? steps : [steps]

  steps.forEach(function(step){
    var provisioner = step.provisioner
    var plugin = provisioner.plugin || 'Shell'
    provisioner.name = step.name || plugin
...
Container.prototype.runTasks = function(){
  return self.buildTasks().each(function(task){
    return task.run().then(function(result){
      // result: {stream, exec (Promise), provisioner}
...
Container.prototype.runBuild = Promise.promisify(function(done) {
  ...
  self.create(function(error, containerData) {
    ...
    return self.runTasks().nodify(done)
  });
});
