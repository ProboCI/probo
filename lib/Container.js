...
Container.prototype.getDiskUsage = Promise.promisify(function(done) {
  ...
  this.container.inspect(function(err, info) {
    ...
    self.containerSize(function(err, containerSize) {
      result.containerSize = err ? err : containerSize;
      self.imageSize(info.Image, function(err, imageSize) {
...
/**
 * Gets the disk size of the container layer for this container in bytes.
 * NOTE: Only supported if the driver is aufs!
 *
 * @param {Function} done - callback that takes err and the container size in bytes as arguments.
 */
Container.prototype.containerSize = function(done) {
  var self = this;
  var command = `du -bs /var/lib/docker/aufs/diff/${this.containerId}`;
  child_process.exec(command, function(error, stdout, stderr) {
    if (error) {
      ...
      self.log.error(err, 'Failed to get container layer size: ' + error.message);
      return done(error);
    }
    ...
    // in bytes
    var containerSize = +stdout.split(/\s/)[0];
...
/**
 * Gets the disk size of the container's image virtual size in bytes.
 * @param {String} imageId - Image id whose size to get
 * @param {Function} done - callback that takes err and the image size in bytes as arguments.
 */
Container.prototype.imageSize = function(imageId, done) {
  var self = this;
  this.docker.getImage(imageId).inspect(function(error, info) {
    if (error) {
      var err = {
        err: error,
        container_id: self.containerId,
      ...
      self.log.error(err, 'Failed to get image size: ' + error.message);
      return done(error);
    }

    // in bytes
    var imageSize = info.VirtualSize;
