 ...
 */
API.prototype.setBuildStatus = function(buildId, context, status, cb){
  ...
  var self = this;
  this._http("/builds/" + buildId + "/status/" + context, "post").send(status).end(function(err, res){
    if(err){
      self.log.error({ err: err, build_id:buildId }, 'An error occurred updating build status');
      return cb && cb(err);
    }

    var status = res.body;
    self.log.info({status: status}, 'Build status updated for GH request' + request.id);
