      ...
      try {
        ...
        result.streams.combined
          ...
          .pipe(through2(function(chunk, enc, cb) {
            ...
            // Timestamp for this needs to go here if the PHP routine isn't good
            // enough on our web app end. This is not perfect but tasks do not
            // have a timestamp associated with them.
            const taskTime = microtime.nowDouble();
            opts.producer.stream.write({topic: opts.producer.config.topics.step, command: chunk.toString().trim(), build_id: task.container.build.id, step_id: task.id, timestamp: taskTime, step_name: task.name});
            cb();
          }));
