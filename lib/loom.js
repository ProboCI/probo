...
function createClient(config, log) {
  config = config || {};
  if (!config.url) {
    throw new Exception('url config required for loom client');
  }

  log = log ? log.child({component: 'loom'}) : require('./logger').getLogger('loom');
  ...
  return {
    ...
    createLogStream: function(metadata, opts) {
      ...
      loom.on('error', function(err) {
        _log.warn({err: err}, 'Loom socket closed');
      });
