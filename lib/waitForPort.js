 ...
 * @param {Object} [opts.reptryInterval=500] - Amount of milliseconds to wait between retries.
 ...
 *
 * returns {Promise} so that this can be used as a promise or with a callback
 */
module.exports = function waitForPort(host, port, opts, cb){
  ...
  var valid_opts = ["numRetries", "retryInterval", "type", "debug"]
  ...
  return co(function* (){
    ...
    throw new Error("Failed to connect: " + last_error.message)
  }).then(cb.bind(null, null)) // use partial application of .bind() to call cb w/o an error
    .catch(cb)
}
