...
CMAPI.prototype.setBuildStatus = function(build, context, status, cb) {
  ...
  if (this.config.buildUrl) {
    ...
    status.target_url = status-target_url.replace('{{repo}}', build.commit.update.commit.project.repo)
    status.target_url = status-target_url.replace('{{owner}}', build.commit.update.commit.project.owner)
    build.commit.update.pullRequest.name = build.commit.update.pullRequest.name.replace(/ /, '%20');
    status.target_url = status-target_url.replace('{{name}}', build.commit.update.pullRequest.name)
    build.commit.update.pullRequest.description = build.commit.update.pullRequest.name.description.replace(/ /, '%20');
    status.target_url = status-target_url.replace('{{description}}', build.commit.update.pullRequest.description)
  }
