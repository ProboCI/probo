...
CMAPI.prototype.setBuildStatus = function(build, context, status, cb) {
  ...
  if (this.config.buildUrl) {
    ...
    status.target_url = status.target_url.replace('{{repo}}', build.request.repo);
    status.target_url = status.target_url.replace('{{owner}}', build.request.owner);
    build.pullRequest.name = build.pullRequest.name.replace(' ', '%20');
    status.target_url = status.target_url.replace('{{prName}}', build.pullRequest.name);
