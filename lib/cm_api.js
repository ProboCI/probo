...
CMAPI.prototype.setBuildStatus = function(build, context, status, cb) {
  ...
  if (this.config.buildUrl) {
    ...
    status.target_url = status.target_url.replace('{{service}}', build.request.service);
  ...
  if (this.config.endpointUrl) {
    var request = {
      build_id: build.id,
      repo: build.project.repo,
      owner: build.project.owner,
      service: build.request.service,
      pull_request_url: build.request.pull_request_html_url,
    ...
    myRequest({
      ...
      this.log.error({err: error}, 'Could not send message to endpoint.');
    });
  }
