...
Server.prototype.runBuild = function* (build, log) {
  ...
  let image = build.config.image || 'proboci/ubuntu:php8.3';
  const type = build.config.type || 'lamp';
  const dist = build.config.dist || 'ubuntu';
  const php = build.config.php || '8.3';
  const database = build.config.database || null;
  const solr = build.config.solr || null;

  if (type === 'lamp' && !image && php) {
    image = 'proboci/' + dist + ':php' + php;
  }
