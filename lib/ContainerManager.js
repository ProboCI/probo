...
var log_container_events = function (container){
  var events = ["running tasks", "tasks complete", "creating", "stopping", "restarting",
                "starting", "removing", "created", "stopped", "restarted", "started",
                "removed"]
  for(let event of events){
    container.on(event, function(err, data){
    ...
    co(function*(){
      ...
      function stateReducer(running){
        return function(count, info){
          var state = info.state || info.State
          if(running)
...
/**
 * Update the container status for all builds. Intended as a one-time catchup to
 * add metadata set by updateBuildContainerStatus()
 */
Server.prototype.routes.post['updatebuilds'] = function(req, res) {
  var self = this
