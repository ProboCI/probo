...
Server.prototype.routes.post['container/proxy'] = function(req, res, next) {
  ...
  if (!buildId.match(uuidRegex)) {
    ...
    res.send(404, {errorCode: '404I', message: 'Build id is invalid'});
    return next();
  }
  ...
  co(function* () {
    ...
    try {
      ...
      if (err.notFound) {
        res.setHeader('content-type', 'application/json');
        res.send(404, {errorCode: '404N', message: `Build not found for build id: ${buildId}`});
        return next();
      }
    }
    ...
    if (!build.container) {
      res.setHeader('content-type', 'application/json');
      res.send(404, {errorCode: '404P', message: 'Build is still in progress'});
      return next();
    }

    if (build.reaped) {
      res.setHeader('content-type', 'application/json');
      res.send(404, {errorCode: '404R', message: 'Build has been reaped'});
      return next();
    }
