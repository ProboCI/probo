 ...
 * GET /container?bid=<buildId>
 */
Server.prototype.routes.post['container'] = function(req, res, next) {
  var self = this

  var buildId = req.query.bid
  if(!buildId){
    return next(new restify.InvalidContentError("bid query param required"));
  }
  ...
  co(function* (){
    ...
    req.log.info({build}, "This is the build")
    ...
    try {
      ...
      if(!state.Running){
        ...
        // TODO: make this timeout configurable and automatically
        // extendable if it gets traffic (through the proxy)
        var stopTimeout = 10 * 60 * 1000 // 10 min

        // stop container again after a certain timeout
        setTimeout(function(){
          req.log.info({id: container.containerId}, "Stopping container")
          container.stop()
       }, stopTimeout)
      } else {
        // 𝅘𝅥𝅮𝅘𝅥𝅮𝅘𝅥𝅮
      ...
      var exposedHostPort = container.getExposedPortFromContainerInfo(inspect, 80)
      ...
      delete url_parts.host  // if host is set, port isn't used in .format(0
      ...
      // problemcs interacting with the container
      req.log.error({err: e}, "Problem stating or starting container")
