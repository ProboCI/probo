...
var emitBuildEvent = function(build, event, data, opts) {
  ...
  if (opts.producer) {
    opts.producer.stream.write({topic: opts.producer.config.topics.event, event, data, build});
  }
};
...
Server.prototype.configure = function(config, done) {
  ...
  this.log = logger.get('container-manager');
  if (config.logLevel) {
  ...
  self.buildEventsProducer = new eventbus.plugins[config.buildsEventStream.plugin].Producer(config.buildsEventStream.config);
...
Server.prototype.runBuild = function* (build, log) {
  ...
  if (!build.config.image) {
    var type = build.config.type || 'lamp';
    var php = build.config.php || '8.3';
    var dotnet = build.config.dotnet || '9';
    var database = build.config.database || null;
    var solr = build.config.solr || null;
    var nodejs = build.config.nodejs || null;

    if (type === 'lamp') {
      image = 'proboci/ubuntu:php' + php;
    } else if (type === 'dotnet') {
      if (dotnet < 6 || dotnet > 9) {
        dotnet = '9';
      }
      image = 'proboci/ubuntu:dotnet' + dotnet;
    } else if (type === 'nodejs') {
      image = 'proboci/ubuntu:nodejs';
    } else {
  ...
  var containerName = `${self.config.containerNamePrefix}--${project.slug.replace(/\//g, '.')}--${project.id}--${build.id}`;
  ...
  var setupTasks = yield container.buildSetupTasks(self.config);
  ...
  setImmediate(function() {
    co(function* () {
      ...
      try {
        ...
        yield* runTasks(setupTasks, {log: log, container: container, loom: self.loom, setup: true, producer: self.buildEventsProducer});
      ...
      if (setupSuccessful) {
        ...
        yield* runTasks(userTasks, {log: log, container: container, loom: self.loom, producer: self.buildEventsProducer});
      }
...
Server.prototype.routes.post['container/proxy'] = function(req, res, next) {
  ...
  if (!buildId) {
    return next(new restifyErrors.InvalidContentError('build query param required'));
  }
  ...
  co(function* () {
    ...
    try {
      ...
      return next(new restifyErrors.NotFoundError('Could not get container, error: ' + e.message));
    }
  }, {req, res, next});
};
...
Server.prototype.routes.get['builds/:id'] = function(req, res, next) {
 ...
 * Closes the server.
 *
 * This is mostly for testing purposes since mocha won't exit if there are open
 * connection pools.
 */
