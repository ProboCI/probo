...
/**
 * TODO: This class currently does far too much. It is both the controller
 * (the HTTP routes) as well as the actual logic to interact with the container
 * management processes and decoupling the HTTP is going to be a little tricky.
 */
...
var stepPlugins = require('./plugins/Step');
...
class Server {
  ...
  constructBuild(buildConfig) {
    console.log('\n************************************************* starting\n', buildConfig, '/n************************************************* ending\n');
    // TODO: This initialization process seems kind of messy, I think we can clean it up.
    var containerConfig = this.createContainerConfig(buildConfig);
    var container = new Container(containerConfig);
    var build = new Build({id: buildConfig.id, commit: buildConfig.commit, project: buildConfig.project, userConfig: buildConfig.config});
    container.build = build;
    build.container = container;
    var stepFactory = new StepFactory({build, container, stepPlugins});
    // TODO: This is getting weird.
    // Add pre-build steps.
    this.log.error(this.config.preBuildSteps, 'pre steps config');
    this.log.error(this.config.postBuildSteps, 'post steps config');
    var steps = stepFactory.createStepsFromConfig(this.config.preBuildSteps);
    // Add user configured steps.
    steps = steps.concat(stepFactory.createStepsFromConfig(buildConfig.config.steps));
    // Add post-build steps.
    steps = steps.concat(stepFactory.createStepsFromConfig(this.config.postBuildSteps));
    var stepList = new StepList(container, {steps});
    build.step = stepList;
    var self = this;
    var getLogger = function(type) {
      return function(data) {
        ...
        self.log.info(logData, `step ${type}ed`);
      };
    };
    build.on('stepStart', getLogger('start'));
    build.on('stepEnd', getLogger('end'));
    return build;
  }

  runBuild(buildConfig, log, done) {
    var self = this;

    buildConfig.log = log.child({}, true);
    var build = this.constructBuild(buildConfig, log);
    build.stream.pipe(through2.obj(function(data, enc, cb) {
      self.log.info('Command output: ' + data.data);
      cb(null, data);
    }));

    var project = buildConfig.project;

    var containerName = `${self.config.containerNamePrefix}--${project.slug.replace('/', '.')}--${project.id}--${buildConfig.id}`;
    buildConfig.containerName = containerName;

    log.info(`Starting container: ${containerName}`);

    build.container.create(function(error, data) {
      // TODO: Refactor codeDownloader into a class that switches for the type (or something).
      if (error) return done(error);
      build.run(function(error, data) {
        if (error) return done(error);
        var returnData = {
          status: 'build started',
          container: {
            id: build.container.id || null,
            name: containerName,
          },
          build: {
            id: build.id,
          },
          steps: build.step.steps.length,
        };
  ...
  resetContainerIdleTimeout(container, set, log) {
 ...
 * // TODO: Consider renaming these actualSize and virtualSize, I would have guessed wrong about this.
 * disk:
...
Server.prototype.routes.post['start-build'] = function(req, res, next) {
  ...
  req.log.debug(body, `Starting build ${build.id}`);

  req.buildId = build.id;
  build.project = project;
  var self = this;

  // TODO: Is there a better way to handle the logger?
  self.runBuild(build, req.log, function(error, data) {
    if (error) {
