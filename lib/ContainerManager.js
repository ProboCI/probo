...
Server.prototype.configure = function(config, done) {
  ...
  try {
    new eventbus.plugins.Kafka.Producer(
      {topic: 'build_events', version: 1},
 ...
 * diskSpace:
 *  .virtual - size of the container's flattened images in bytes, if container exists, null otherwise
 *  .real - size of the container's own RW layer in bytes, if container exists, null otherwise
 */
