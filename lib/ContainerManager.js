...
Server.prototype.configure = function(config, done) {
  ...
  if (config.jsonWebTokenPublicKey) {
   server.use(this.jsonWebTokenParser(config.jsonWebTokenPublicKey));
  }
...
Server.prototype.jsonWebTokenParser = function(publicKey) {
  var handleError = function(res) {
    res.status(401);
    res.json({error: 'Invalid JWT provided'});
  };
  return function(req, res, next) {
    ...
    jwt.verify(token, publicKey, {algorithms: ['RS256']}, function(error, data) {
      if (error) {
        req.log.error(error);
