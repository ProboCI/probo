...
var Steps = require('./plugins/Step');
var StepList = Steps.StepList;
var Shell = Steps.Shell;
...
class Server {
  ...
  constructBuild(buildConfig, log) {
    log = log.child({bid: buildConfig.id}, true);
    var containerConfig = this.createContainerConfig(buildConfig, log);
    containerConfig.log = log;
    var container = new Container(containerConfig);
    var build = new Build();
    // TODO: make this auto-reference fancier?
    ...
    var shellOptions = {
      name: 'Shell Test',
      command: 'mkdir /var/www/html && echo "<h1>Hello world.</h1>" > /var/www/html/index.html && chmod 777 -R /var/www/html',
    };
    stepList.addStep(new Shell(container, shellOptions));
    return build;
  }
  ...
  runBuild(buildConfig, log, done) {
    ...
    log.info(`Starting container: ${containerName}`);
      ...
      // TODO: Move construction of build into its own method or even helper class.
