...
Server.prototype.routes.post['startbuild'] = function(req, res, next) {
  ...
  co(function* (){
    ...
    yield updateEnvStatus({state: "pending", description: "[running] The hamsters are working hard on your setup"})
    for(let task of tasks){
      ...
      yield task.updateStatus({state: "pending", description: `[pending] ${task.description()}`})
    }
    ...
    try {
      ...
      yield updateEnvStatus({state: "success", description: "[finished] Environment built"})
    } catch(e){
      switch(e.statusCode){
    ...
    try {
      ...
      for(let task of tasks){
        ...
        yield task.updateStatus({state: "pending", description: `[running] ${task.description()}`})

        let result = yield task.run
        //let task = result.provisioner
