  ...
  co(function* (){
    ...
    try {
      ...
      try {
        ...
        var port_opts = { numRetries: 10, retryInterval: 500}
        yield waitForPort('localhost', exposedHostPort, port_opts)
        req.log.debug(`Container port ${targetPort}/${exposedHostPort} up`)
      } catch (e){
        var duration = (new Date().getTime()) - start
        req.log.warn(`Service connection timed out on container port ${targetPort}/${exposedHostPort} after ${duration}ms. Continuing with proxy response anyway.`)
      }
