...
Server.prototype.routes.post['startbuild'] = function(req, res, next) {
  ...
  co(function* (){
    ...
    var updateStatus = function(){
      ...
      return function* (build, context, status){
        try {
    ...
    yield updateStatus(build, "build", {state: "pending"})

    try {
      steps = yield container.runBuild()
      yield updateStatus(build, "build", {state: "success"})
      ...
      default:
        ...
        yield updateStatus(build, "build", {state: "error", description: e.message})
      }
    }
    ...
    try {
      ...
      for(let step of steps){
        ...
        let task_slug = `${result.provisioner.options.plugin}/${result.provisioner.name}`

        log.debug("Step exec started:", result.provisioner.name)
        yield updateStatus(build, task_slug, {state: "pending"})
        ...
        try {
          ...
          yield updateStatus(build, task_slug, {
          ...
          yield updateStatus(build, task_slug, {state: "error", description: e.message})
        }
      }
