...
Server.prototype.routes.post['startbuild'] = function(req, res, next) {
  ...
  co(function* (){
    ...
    var containerName = `${project.slug.replace('/', '.')}-${project.id}-${build.id}`
    ...
    try {
      ...
      // stream output back to the request (for now)
      res.write("OUTPUT:\n---------\n")
      var chunk
      while((chunk = yield read(last_result.stream))){
        res.write(chunk)
      }
      res.write('\n')

      // dump exit code
      var data = yield last_result.exec // returns result of container.inspect call
      var exit_code = data.ExitCode
      res.write(`EXIT_CODE: ${exit_code}\n`)

      res.end('')
    } catch(error){
      req.log.error({err: error}, "Error processing steps")
