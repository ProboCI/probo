...
var emitBuildEvent = function(build, event, data, opts){
  ...
  if(opts.log){
    opts.log.debug({event, // build, 
                    data}, `build event for build ${build.id}: ${event}`)
  }

  // send the event to the event bus...
  if(opts.producer){
    opts.producer.stream.write({
      event,
      data,
      build,
    })
...
Server.prototype.configure = function(config, done) {
  ...
  try {
    new eventbus.plugins[config.buildsEventStream.plugin].Producer(
      config.buildsEventStream.config,
 ...
 * disk:
 *  .virtualSize - size of the container's flattened images in bytes, if container exists, null otherwise
 *  .realSize - size of the container's own RW layer in bytes, if container exists, null otherwise
 */
