  ...
  setImmediate(function() {
    co(function* () {
      ...
      try {
        ...
        yield self.storeBuildDataAsync(build);
        container.getDiskUsage(function(error, info) {
          var message = {
            id: build.id,
            reaped: false,
            diskSpace: {
              container: info.containerSize,
              image: info.imageSize,
            },
          };
          self.eventProducer.stream.write(message);
        });
...
Server.prototype.routes.del['containers/:id'] = function(req, res, next) {
  ...
  container.inspect(function(err, info) {
    ...
    self.eventProducer.stream.write({
      id: buildId,
      reaped: true,
    });
