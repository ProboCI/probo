...
Server.prototype.runBuild = function* (build, log) {
  ...
  let image = null;
  const type = build.conf.type || 'lamp';
  const dist = build.conf.dist || 'ubuntu';
  const php = build.conf.php || '8.3';
  const database = build.conf.database || null;
  const solr = build.conf.solr || null;
