...
class GitHub {
  ...
  getApi(project) {
    ...
    this.logger.error({options: options}, 'Getting our options.');
    this.logger.error({project: project}, 'Getting our project.');
  ...
  getCommit(project, sha, cb) {
    ...
    github.repos.getCommit({owner: project.owner, repo: project.repo, ref: sha})
      .then(res => {
        const commit = {
          sha: res.data.sha,
   ...
   *
   * @param {Object.<string, any>} project - The project object to post the
   *   status for.
   * @param {string} sha - The git commit id that we are posting a status for.
   * @param {string} statusInfo - This should be the status message to post to
   *   GH. See https://developer.github.com/v3/repos/statuses/
   * @param {(err: Error, [res]) => void} cb - The callback to call when the
   ...
   * @param {number} query.pull_number - The pull request number.
