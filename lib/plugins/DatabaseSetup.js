...
module.exports = function databaseSetup(options) {
  ...
  let script = [];
  if (options.database) {
    script = script.concat(configureDatabase(db, options));
    script = script.concat(restartDatabase(db, options));
    script = script.concat(pingDatabase(db));
    script = script.concat(createDatabase(db, options));
    script = script.concat(importDatabase(db, options));
  }
  return script;
}
...
function createDatabase(database, options) {
  ...
  switch (database) {
    ...
    case 'mariadb':
      ...
      break;
    case 'postgres':
      /** This is done as part of setup so we don't need to do it here */
      break;
    case 'mssql':
...
function importPostgresqlDatabase(options) {
  let script = [];
  script = script.concat(['echo "Importing PostgreSQL database."']);
  let databaseImportBase = '';
  if (options.options.databaseGzipped) {
    databaseImportBase =
      'gunzip -c $ASSET_DIR/' + options.options.database + ' |';
  } else if (options.options.databaseBzipped) {
    databaseImportBase =
      'bunzip2 -c $ASSET_DIR/' + options.options.database + ' | ';
  } else {
    databaseImportBase = 'cat $ASSET_DIR/' + options.options.database + ' | ';
  }
  script.push(
    `${databaseImportBase} $(psql --username=root -w -d probo);`
  );
  script.push(`rm $ASSET_DIR/${options.options.database};`);
  return script;
}
...
function createMariadbDatabase(options) {
  ...
  script = script.concat([
    ...
    'if [ "$DATABASE_USER" != "root" ]; then mariadb -e \'create user "\'$DATABASE_USER\'"@"localhost" identified by "\'$DATABASE_PASS\'"\'; fi',
...
function mysqlDatabaseConfiguration(options) {
  let script = [];
  script = script.concat(
  ...
  if (!isEmptyObject(options.dbOptions)) {
    for (var key in options.dbOptions) {
      if (options.dbOptions.hasOwnProperty(key)) {
        var val = sanitizeValue(options.dbOptions[key]);
        script = script.concat(
...
function mariadbDatabaseConfiguration(options) {
  let script = [];
  script = script.concat(
    'echo "!include /etc/mysql/probo-settings.cnf" >> /etc/mysql/my.cnf'
  );
