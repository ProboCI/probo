...
module.exports = function databaseSetup(database, options) {
  ...
  let script = [];
  if (options.database) {
    script.concat(configureDatabase(database, options));
    script.concat(restartDatabase(database, options));
    script.concat(pingDatabase(database));
    script.concat(createDatabase(database, options));
    script.concat(importDatabase(database, options));
  }
  return script;
}
...
function createDatabase(database, options) {
  ...
  switch (database) {
    case 'mysql':
      script.concat(createMysqlDatabase(options));
      break;
    case 'mariadb':
      script.concat(createMariadbDatabase(options));
      break;
    case 'postgresql':
      script.concat(createPostgresqlDatabase(options));
      break;
    case 'mssql':
      script.concat(createMssqlDatabase(options));
      break;
  }
  return script;
}
...
function importDatabase(database, options) {
  ...
  switch (database) {
    case 'mysql':
      script.concat(importMysqlDatabase(options));
      break;
    case 'mariadb':
      script.concat(importMariadbDatabase(options));
      break;
    case 'postgresql':
      script.concat(importPostgresqlDatabase(options));
      break;
    case 'mssql':
      script.concat(importMssqlDatabase(options));
      break;
  }
  return script;
}
...
function importMysqlDatabase(options) {
  let script = [];
  script.concat(['echo "Importing database."']);
...
function restartDatabase(database, options) {
  ...
  switch (database) {
    case 'mysql':
      script = script.push('service mysql restart');
      break;
    case 'mariadb':
      script = script.push('service mariadb restart');
      break;
    case 'postgresql':
    case 'mssql':
    default:
