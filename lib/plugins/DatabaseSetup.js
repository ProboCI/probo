...
class DatabaseSetup {
  ...
  go() {
    ...
    // If not, then skip it.
    this.script = [];
    this.script.push('Database GO! ' + this.options.database + ':' + this.database);
  ...
  configureDatabase() {
    switch (this.database) {
      case 'mysql':
        ...
        break;
      case 'mariadb':
        this.mariadbDatabaseConfiguration();
        break;
      case 'postgresql':
  ...
  pingDatabase() {
    ...
    switch (this.database) {
      ...
      case 'mariadb':
        ...
        break;
      case 'postgresql':
        this.script = this.script.push('pg_isready');
        break;
      case 'mssql':
        this.script = this.script.push('sqlcmd -S localhost -Q "SELECT 1"');
        break;
    }
  }
