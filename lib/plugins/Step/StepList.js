...
class StepList extends AbstractStep {
  ...
  constructor(container, options) {
    ...
    this.endedStreams = [];
    this.stepErrors = [];
    this.stream.pipe(through2.obj(function(data, enc, cb) { console.log('step list message received....', data.data); cb(null, data); }));

    var self = this;
    this.stream.on('end', function() {
      self.log.error('steplist general stream ended for ' + self.name);
    });
  }
  ...
  streamEndHandler(stream) {
    ...
    // this.log.warn(`${this.name} handler invoked 🍗  ended: ${this.endedStreams.length} steps: ${this.steps.length}`);
