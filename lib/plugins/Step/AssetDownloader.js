...
module.exports = class AssetDownloader extends require('./Script') {
  ...
  constructor(container, options) {
    ...
    // TODO: This *SUCKS* and we need to decide how to more gracefully handle config that needs to
    // be specified outside the step level in the .probo.yaml file.
    options.assets = options.assets || options.build.userConfig.assets;
    options.bucket = options.bucket || options.build.project.assets.bucket;
    ...
    var script = [
      'mkdir -p $ASSET_DIR',
      // TODO: Move this directory setting behavior into a method to modify the container startup settings.
      'cd $ASSET_DIR',
    ];
  ...
  createAssetDownloadCommand(asset) {
    ...
    var auth = this.options.token ? `--header='Authorization:Bearer ${this.options.token}'` : '';
