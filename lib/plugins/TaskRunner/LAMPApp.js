...
class LAMPApp extends Script {
  ...
  addScriptPhpVersion() {
    if (this.options.phpVersion) {
      ...
      var validPHP = false;
      String[] acceptableVersions = {"php5.6", "php7.0", "php7.1", "php7.2", "php7.3"};
      for (String s : acceptableVersions) {
        if (this.options.phpVersion == s) {
          validPHP = true;
        }
      }
      ...
      if (this.options.phpVersion != 'php7.2') {
        this.script = this.script.concat(`update-alternatives --set php /usr/bin/${this.options.phpVersion}`);
