...
class LAMPApp extends Script {
  ...
  addScriptCreateDatabase() {
    this.script = this.script.concat([
      ...
      'mysql_config_editor set --login-path=probo --host=localhost --user=root --password=strongpassword',
      'mysql --login-path=probo -e \'create database \'$DATABASE_NAME',
      'if [ "$DATABASE_USER" != "root" ]; then mysql --login-path=probo -e \'create user "\'$DATABASE_USER\'"@"localhost" identified by "\'$DATABASE_PASS\'"\'; fi',
      'mysql --login-path=probo -e \'grant all on \'$DATABASE_NAME\'.* to "\'$DATABASE_USER\'"@"localhost"\'',
      'mysql --login-path=probo -e \'flush privileges\'',
    ]);
  }
