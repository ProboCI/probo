...
class LAMPApp extends Script {
  ...
  constructor(container, options) {
    ...
    this.databaseUser = options.databaseUser || 'probo';
    ...
    const isMySQL = database === 'mysql' || database === 'mariadb';
    ...
    this.script = [];
    this.populateScriptArray(isMySQL);
   ...
   *
   */
  populateScriptArray(isMySQL) {
    this.addScriptSetup(isMySQL);
  ...
  addScriptSetup(isMySQL) {
    this.addScriptHeader(isMySQL);
  ...
  addScriptPhpIniOptions() {
    ...
    if (this.options.linux == "alpine") {
      const pathsToCheck = ['/etc/php7', '/etc/php8', '/etc/php81', '/etc/php82', '/etc/php83', '/etc/php84', '/etc/php85'];
