...
class LAMPApp extends Script {
  ...
  addScriptCreateDatabase() {
    this.script = this.script.concat([
      ...
      'mysql --defaults-extra-file=/root/.my.cnf -e \'CREATE DATABASE IF NOT EXISTS \'$DATABASE_NAME',
      'if [ "$DATABASE_USER" != "root" ]; then mysql --defaults-extra-file=/root/.my.cnf -e \'CREATE USER "\'$DATABASE_USER\'"@"localhost" IDENTIFIED BY "\'$DATABASE_PASS\'"\'; fi',
      'if [ "$DATABASE_USER" != "root" ]; then mysql --defaults-extra-file=/root/.my.cnf -e \'GRANT ALL ON \'$DATABASE_NAME\'.* TO "\'$DATABASE_USER\'"@"localhost"\'; fi',
      'mysql --defaults-extra-file=/root/.my.cnf -e \'FLUSH PRIVILEGES\'',
    ]);
  }
  ...
  addScriptImportDatabase() {
    if (this.options.database) {
      ...
      this.script.push(`${databaseImportBase} $ASSET_DIR/${this.options.database} | $(mysql --defaults-extra-file=/root/.my.cnf $DATABASE_NAME);`);
