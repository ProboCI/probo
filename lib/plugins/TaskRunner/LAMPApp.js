...
class LAMPApp extends Script {
  ...
  // Function for doing things that can be overridden by child classes
  populateScriptArray() {
    this.addScriptSetup();
    this.addComposerInstall();
    this.addScriptSymlinks();
    this.script = this.script.concat(databaseSetup(this.database, this.options));
  ...
  addScriptHeader() {
    ...
    this.script = this.script.concat(["PHPINI_PATH=\"$(php -i | grep php.ini | head -1 | sed 's/\\/cli//g' | sed 's/.* //g')\""]);
  }
  ...
  addComposerInstall() {
    if (this.options.composer === true) {
      this.script = this.script.push('cd $SRC_DIR');
      this.script = this.script.push('composer -n install');
      this.script = this.script.push('cd');
    }
  }
