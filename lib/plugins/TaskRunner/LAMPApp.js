...
class LAMPApp extends Script {
  ...
  addScriptCreateDatabase() {
    this.script = this.script.concat([
      '# Creating MySQL Database, user and granting access.',
  ...
  addScriptImportDatabase() {
    if (this.options.database) {
      ...
      this.script = this.script.concat(['# Importing database.']);
      this.script.push(
        `pv -f $ASSET_DIR/${this.options.database} | ${databaseImportBase} | $(mysql -u $DATABASE_USER --password=$DATABASE_PASS $DATABASE_NAME);`
      );
