...
class LAMPApp extends Script {
  ...
  constructor(container, options) {
    ...
    this.databaseEngine = options.databaseEngine || 'mysql';
    ...
    this.options.phpVersion = options.phpVersion || 'php7.3';
  ...
  // Only MySQL and PostgreSQL are allowed. Make sure only one of those two are specified.
  addScriptValidateRDBMS() {
    if (this.databaseEngine != 'mysql' && this.databaseEngine != 'pgsql') {
      this.script.push('echo \'Invalid database ' + this.databaseEngine + '. Only MySQL (mysql) or PostgreSQL (pgsql) allowed.\'');
  ...
  addScriptCreateDatabase() {
    this.script = this.script.concat([
      ...
      'mysql --defaults-extra-file=/root/.probo.cnf -e \'FLUSH PRIVILEGES\'',
      'mysql --defaults-extra-file=/root/.probo.cnf -e \'CREATE DATABASE IF NOT EXISTS \'$DATABASE_NAME',
      'if [ "$DATABASE_USER" != "root" ]; then mysql --defaults-extra-file=/root/.probo.cnf -e \'CREATE USER "\'$DATABASE_USER\'"@"localhost" IDENTIFIED BY "\'$DATABASE_PASS\'"\'; fi',
      'if [ "$DATABASE_USER" != "root" ]; then mysql --defaults-extra-file=/root/.probo.cnf -e \'GRANT ALL ON \'$DATABASE_NAME\'.* TO "\'$DATABASE_USER\'"@"localhost"\'; fi',
      'mysql --defaults-extra-file=/root/.probo.cnf -e \'FLUSH PRIVILEGES\'',
    ]);
  }
  ...
  addScriptImportDatabase() {
    if (this.options.database) {
      ...
      this.script.push(`${databaseImportBase} $ASSET_DIR/${this.options.database} | $(mysql --defaults-extra-file=/root/.probo.cnf $DATABASE_NAME);`);
  ...
  addScriptPhpVersion() {
    if (this.options.phpVersion) {
      ...
      var validPHP = false;
      var acceptableVersions = ['php5.6', 'php7.0', 'php7.1', 'php7.2', 'php7.3', 'php7.4', 'php8.0'];
      var index = 0;
      ...
      // If no valid version, use default (currently php7.3)
      if (validPHP == false) {
        this.options.phpVersion = 'php7.3';
      }
  ...
  addScriptInstallPackages() {
    ...
    if (!this.isEmptyObject(packages)) {
      ...
      this.script = this.script.concat('apt-get -y update');
      this.script = this.script.concat('apt-get -y install ' + packageList);
      this.options.restartApache = true;
    }
  }
