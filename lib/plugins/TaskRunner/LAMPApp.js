...
class LAMPApp extends Script {
  ...
  addScriptCreateDatabase() {
    this.script = this.script.concat([
      ...
      'mysql -uroot -pstrongpassword -e \'create user "\'$DATABASE_USER\'"@"localhost" identified by "\'$DATABASE_PASS\'"\'',
  ...
  addScriptPhpIniOptions() {
    if (!this.isEmptyObject(this.options.phpIniOptions)) {
      ...
      for (let key in paths) {
        let path = paths[key];
        if (options[key] != undefined && !this.isEmptyObject(options[key])) {
  ...
  addScriptPhpVersion() {
    if (this.options.phpVersion) {
      ...
      if (this.options.phpVersion != 'php7.2') {
        ...
        this.script = this.script.concat('service apache2 restart');
      }
    }
  }
