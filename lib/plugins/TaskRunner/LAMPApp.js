...
class LAMPApp extends Script {
  ...
  addScriptHeader() {
    this.script = this.script.concat([
      'READY=0; MYSQL_ALIVE=`mysqladmin ping`; while [ "$MYSQL_ALIVE" != "mysqld is alive" ] && [ $READY -lt 60 ]; do echo "Waiting for MySQL..."; service mariadb start; READY=$((READY + 1)); sleep 1; MYSQL_ALIVE=`mysqladmin ping`; done; if [ "$MYSQL_ALIVE" != "mysqld is alive" ]; then echo "MySQL failed to start!"; exit 1; fi;',
  ...
  addScriptRestartMysql() {
    if (this.options.restartMysql) {
      this.script = this.script.concat([
        'export USER=`valid www-data`',
        'if [[ $USER = 1 ]]; then service mariadb restart; else mysqladmin refresh && mysqladmin reload; fi',
      ]);
    }
  }
