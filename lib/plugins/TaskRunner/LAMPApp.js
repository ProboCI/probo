...
class LAMPApp extends Script {
  ...
  addScriptMysqlCnfOptions() {
    ...
    this.script = this.script.concat(`echo "[mysqld]" >> ${constants.MYSQLCNF_PATH}/probo-settings.cnf`);
    if (!this.isEmptyObject(options)) {
      for (var key in options) {
        if (options.hasOwnProperty(key)) {
          var val = this.sanitizeValue(options[key]);
          this.script = this.script.concat(`echo "${key}=${val}" >> ${constants.MYSQLCNF_PATH}/probo-settings.cnf`);
        }
      }
