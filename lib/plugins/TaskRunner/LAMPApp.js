...
class LAMPApp extends Script {
  ...
  addScriptMysqlCnfOptions() {
    ...
    this.script = this.script.concat(`echo "!include ${constants.MYSQL_PATH}/probo-settings.cnf" >> ${constants.MYSQL_PATH}/my.cnf`);
    this.script = this.script.concat(`echo "[mysqld]" >> ${constants.MYSQL_PATH}/probo-settings.cnf`);
    if (!this.isEmptyObject(options)) {
      for (var key in options) {
        if (options.hasOwnProperty(key)) {
          var val = this.sanitizeValue(options[key]);
          this.script = this.script.concat(`echo "${key}=${val}" >> ${constants.MYSQL_PATH}/probo-settings.cnf`);
        }
      }
