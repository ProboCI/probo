...
class LAMPApp extends Script {
  ...
  constructor(container, options) {
    ...
    // This is the database that is assigned to the container
    this.database = container.database;

    // This is the database file to be imported from the plugin
    this.options.database = options.database;

    this.options.databaseName = options.databaseName || 'lampdb';
    this.options.databaseUser = options.databaseUser || 'probo';
    this.options.databasePass = options.databasePass || 'strongpassword';
    ...
    // Our database setup class.
    this.databaseSetup = new DatabaseSetup(this.database, this.options);

    this.addScriptSetup();
    this.addComposerInstall();
    this.addScriptSymlinks();
    this.script = this.script.concat(this.databaseSetup.go());
  ...
  // Boy oh boy does this need a re-factor. This fires up MySQL (or makes sure it is fired up)
  // before continuing with the rest of the script. We may want to purpose this into an include
  ...
  addScriptHeader() {
    ...
    this.script.push("PHPINI_PATH=\"$(php -i | grep php.ini | head -1 | sed 's/\\/cli//g' | sed 's/.* //g')\"");
  }
  ...
  // git repository. This function is different from the Drupal one in that this one does not
  // require Drupal or a specific version of Drupal.
  addComposerInstall() {
    if (this.options.composer === true) {
