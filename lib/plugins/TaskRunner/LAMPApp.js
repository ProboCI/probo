...
class LAMPApp extends Script {
  ...
  addScriptHeader() {
    this.script = this.script.concat([
      'READY=0; MYSQL_ALIVE=`mysqladmin ping`; while [ "$MYSQL_ALIVE" != "mysqld is alive" ] && [ $READY -lt 60 ]; do echo "Waiting for MySQL..."; READY=$((READY + 1)); sleep 1; MYSQL_ALIVE=`mysqladmin ping`; done; if [ "$MYSQL_ALIVE" != "mysqld is alive" ]; then echo "MySQL failed to start!"; exit 1; fi;',
  ...
  // Ubuntu and Alpine handle their php.ini files a lot differently from each other.
  // What we need is a flag to tell the Probo daemon when we're using Alpine so we
  // can adust. For the time being, we will just look for the file in a location and
  // if it is not there, we will look for it in the other.
  addScriptPhpIniOptions() {
    let iniPath = false;
    const pathsToCheck = ['/etc/php7', '/etc/php8'];
    for (let path in pathsToCheck) {
      ...
      if (!this.isEmptyObject(this.options.phpIniOptions)) {
        ...
        this.options.restartApache = true;
        const newOptions = { ...options.all, ...options.apache2 };
  ...
  addScriptRestartMysql() {
    if (this.options.restartMysql) {
      this.script = this.script.concat([
        'export USER=`valid www-data`',
        'if [[ $USER = 1 ]]; then service mysqld restart; else mysqladmin refresh && mysqladmin reload; fi',
      ]);
    }
  }
