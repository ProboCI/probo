...
class LAMPApp extends Script {
  ...
  constructor(container, options) {
    ...
    options.phpIniOptions = options.phpIniOptions || {};
    this.options.phpIniOptions = {
      all: options.phpIniOptions.all || {},
      apache2: options.phpIniOptions.apache2 || {},
      cli: options.phpIniOptions.cli || {},
    };
  ...
  addScriptPhpIniOptions() {
    var phpIniOptions = this.options.phpIniOptions;
    if (!this.isEmptyObject(phpIniOptions.all) || !this.isEmptyObject(phpIniOptions.apache2) || !this.isEmptyObject(phpIniOptions.cli)) {
      // We clone the object because we modify it
      // and will mess things up if this is run
      // multiple times.
      let options = JSON.parse(JSON.stringify(phpIniOptions));
      let paths = {
        all: ['apache2', 'cli'],
        apache2: ['apache2'],
        cli: ['cli'],
