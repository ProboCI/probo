...
module.exports = class SOLR extends require('./Script') {
  ...
  constructor(container, options) {
    ...
    script.push('sudo -u solr tar xzf solr-' + ver + '.tgz solr-' + ver + '/bin/install_solr_service.sh --strip-components=2');
    script.push('sudo -u solr ./install_solr_service.sh solr-' + ver + '.tgz');
    script.push('echo \'SOLR_MODULES="analysis-extras"\' > /opt/solr/bin/solr.in.sh');
    
    script.push('sudo -u solr /opt/solr/bin/solr restart -force');
