...
class NodeJSApp extends Script {
  ...
  constructor(container, options) {
    ...
    this.appBuild = options.buildCommand || 'npm i';
    this.appRun = options.runCommand ||  'npm run dev';
    this.environment = options.environment || {};
    this.packages = options.globalPackages || {};
    this.database = options.database || false;
    this.databaseGzipped = options.databaseGzipped || false;
    this.databaseBzipped = options.databaseBzipped || false;

    // Compatibility with our database setup script
    options.database = container.database;
    options.options = this;

    this.script = [];
    this.script = this.script.concat(databaseSetup(options));
  ...
  setEnvironmentVariables() {
    ...
    if (!this.isEmptyObject(envVariables)) {
      ...
      string += this.appRun + "' > /src/startup.sh";
    }
