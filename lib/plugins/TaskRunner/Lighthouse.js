...
module.exports = class Lighthouse extends Script {
  ...
  installLighthouse() {
    ...
    // TODO: Can we include this in images?

    this.script = this.script.concat([
      'set +ux', // Don't be so verbose in logs that we can't even find results.
      'echo "Installing Lighthouse"',
      'npm cache clean -f &> /dev/null',
      'npm install -g n &> /dev/null',
      'n stable &> /dev/null',
      'npm install -g lighthouse &> /dev/null',
      'apt-get install jq &> /dev/null', // Install jq for parsing json.
    ]);
  }
  ...
  parseCategory(value) {
    ...
    this.script = this.script.concat([
      ...
      'if [ $pass -eq 1 ]; then exit 1; fi', // Step fails.
    ]);
  };
