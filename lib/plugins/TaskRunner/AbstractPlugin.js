...
AbstractPlugin.prototype.run = function(done) {
  ...
  function error_handler(err, msg){
    msg = msg || err.message
    ...
    var wrapped_error = new Error(msg)
    wrapped_error.original_error = err
    return done(wrapped_error);
  }
  ...
  container.exec(self.buildOptions(), function(error, exec) {
    ...
    if(error) {
      ...
      return error_handler(error, `${self.name}: Failed to exec command: ${error.message}`)
    }

    exec.start({stream: true, stdin: true, stdout: true}, function(error, stream) {
      log.info('exec started');
      if(error) {
        return error_handler(error, `${self.name}: Failed to start command: ${error.message}`)
      }
