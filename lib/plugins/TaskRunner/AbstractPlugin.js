...
class AbstractPlugin extends events.EventEmitter {
  ...
  retryRun(error, done) {
    var self = this;
    self.log.error(this.getLogErrorContext(error), `${self.name}: Got an EPIPE error, retrying in 2 sec...`);
    setTimeout(function() {
      self.run(done);
    }, 2000);
  }

  getLogErrorContext(error) {
    var context = this.logContext;
    context.err = error;
    return context;
  }
  ...
  run(done) {
    ...
    container.exec(self.buildOptions(), function(error, exec) {
      ...
      exec.start({stream: true, stdin: true, stdout: true}, function(error, stream) {
        ...
        if (error) {
          return self.errorHandler(error, `${self.name}: Failed to start command: ${error.message}`, done);
        }
