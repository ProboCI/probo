...
class Backdrop extends LAMPApp {
  ...
  appendBackdropSettings() {
    this.script = this.script.concat([
      ...
      //`mkdir -p /src/${this.subDirectory}/config/active`,
      //`mkdir -p /src/${this.subDirectory}/config/staging`,
      //`chown -R www-data:www-data /src/${this.subDirectory}`,
    ]);
  }
  ...
  addScriptRunInstall() {
    ...
    this.script.push('chown -R www-data:www-data /src');
  }
}
