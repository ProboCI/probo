...
class Drupal extends LAMPApp {
   ...
   *   @param {integer} options.drushVersion - Set the drush version to be installed, which will define what drush version to install and what drush binary command to use.
  ...
  constructor(container, options) {
    ...
    this.options.drushVersion = options.drushVersion || constants.DEFAULT_DRUSH_VERSION;
  ...
  addScriptRunMakeFile() {
    ...
    this.script.push('cd $SRC_DIR ; drush make ' + this.options.makeFile + ' /var/www/html ' + makeArgs);
  ...
  addScriptFileProxy() {
    if (this.options.drupalVersion >=8) {
      this.script = this.script.concat(
        `cd /src/${this.subDirectory} && cd .. && composer require drupal/stage_file_proxy`,
        `drush --root=/var/www/html en stage_file_proxy -y`,
        `drush --root=/var/www/html -y cset stage_file_proxy.settings hotlink 1`,
        `drush --root=/var/www/html -y cset stage_file_proxy.settings origin '${this.options.fileProxy}'`
      );
    }
    if (this.options.drupalVersion <=7) {
      this.script = this.script.concat(
        `drush --root=/var/www/html dl stage_file_proxy -y`,
        `drush --root=/var/www/html en stage_file_proxy -y`,
        `drush --root=/var/www/html -y vset stage_file_proxy_hotlink 1`,
        `drush --root=/var/www/html -y vset stage_file_proxy_origin '${this.options.fileProxy}'`
      );
    }
  }
