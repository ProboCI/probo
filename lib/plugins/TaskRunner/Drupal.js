...
module.exports = class Drupal extends require('./Script') {
   ...
   *   @param {string} options.installProfile - If set this install (incompatible with the `database` option).
  ...
  constructor(container, options) {
    ...
    this.options.profileName = options.clearCaches != undefined ? options.clearCaches : true
  ...
  addScriptRunMakeFile() {
    ...
    // TODO: I think this is a WTF - what do we do about it?
    this.script.push('rm -rf /var/www/html/profiles/' + this.options.profileName)
    this.script.push('ln -s $SRC_DIR /var/www/html/profiles/' + this.options.profileName)
  }

  addScriptClearCaches() {
    this.script.push('druch --root=/var/www/html cache-clear all')
  }
