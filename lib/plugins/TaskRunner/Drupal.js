...
class Drupal extends LAMPApp {
  ...
  constructor(container, options) {
    ...
    this.databaseName = options.databaseName || constants.DRUPAL_DATABASE_NAME;
    ...
    if (this.databaseEngine == 'pgsql') {
      this.nameSpace = 'Drupal\\\\\\\\Core\\\\\\\\Database\\\\\\\\Driver\\\\\\\\mysql';
    }
    else {
      this.nameSpace = 'Drupal\\\\\\\\Core\\\\\\\\Database\\\\\\\\Driver\\\\\\\\pgsql';
    }
  ...
  addD7PHPSettings() {
    this.script = this.script.concat([
      ...
      `if [ "${this.options.baseURL}" != "" ] ; then`,
      `  echo '$base_url="${this.options.baseURL}";' >> /var/www/html/sites/${this.options.siteFolder}/settings.php`,
      'fi',
    ]);
  }
  ...
  addScriptRunInstall() {
    ...
    this.script.push(`drush site-install -y --root=/var/www/html ${this.options.profileName} ${installArgs}`);
  }

  addScriptDatabaseUpdates() {
    this.script.push('drush --root=/var/www/html updb -y');
  }

  addScriptRevertFeatures() {
    this.script.push('drush --root=/var/www/html -y q');
  }
  ...
  addScriptRunMakeFile() {
    ...
    this.script.push('cd $SRC_DIR ; drush make -y ' + this.options.makeFile + ' /var/www/html ' + makeArgs);
  ...
  addScriptClearCaches() {
    if (this.options.drupalVersion >= 8) {
      this.script.push('drush --root=/var/www/html -y cache-rebuild');
    }
    else {
      this.script.push('drush --root=/var/www/html -y cache-clear all');
    }
  }
