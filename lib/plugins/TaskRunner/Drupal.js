...
class Drupal extends LAMPApp {
  ...
  addScriptSanitizeDatabase() {
    this.script.push('ln -s /modules /var/www/html/modules/probo');
    ...
    if (this.options.revisionContentTypes) {
      this.script.push('drush -y --root=/var/www/html pm:enable node_revision_delete');
      this.script.push(`drush -y --root=/var/www/html nrd-q --type=${this.options.revisionContentTypes}`);
      this.script.push('drush -y --root=/var/www/html queue:run node_revision_delete');
      this.script.push('drush -y --root=/var/www/html cron');
    }
  }
