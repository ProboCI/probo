...
class Drupal extends LAMPApp {
  ...
  populateScriptArray() {
    ...
    if (this.composer === true && this.options.drupalVersion >= 8) {
    ...
    // Do our database work via our standard helpers
    this.script = databaseSetup(this.database, this.options);
  ...
  appendCustomSettings() {
    if (this.options.settingsRequireFile) {
      let command =
        ...
        '/settings.php';
      this.script = this.script.push(command);
    }
  ...
  addScriptDatabaseUpdates() {
    this.script = this.script.push('drush -y --root=/var/www/html updb');
  }

  addScriptRevertFeatures() {
    this.script = this.script.push('drush -y --root=/var/www/html fra');
  }
  ...
  addScriptClearCaches() {
    if (this.options.drupalVersion >= 8) {
      this.script = this.script.push('drush -y --root=/var/www/html cr');
    } else {
      this.script = this.script.push('drush -y --root=/var/www/html cc all');
    }
  }
  ...
  addScriptUnsupportedDrupalVersion() {
    this.script = this.script.push('echo "Build stopped: Unsupported Drupal version."');
    this.script = this.script.push('exit 1');
  }
