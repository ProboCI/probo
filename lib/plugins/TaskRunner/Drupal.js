...
class Drupal extends LAMPApp {
  ...
  constructor(container, options) {
    ...
    for (var key in this.sites) {
      if (this.sites.hasOwnProperty(key)) {
        ...
        siteOptions.options.siteFolder = siteOptions.options.siteFolder || 'default';
        ...
        siteOptions.databaseName = siteOptions.siteData.databaseName || constants.DRUPAL_DATABASE_NAME;
        siteOptions.multisiteName = key;
        this.sites[key] = siteOptions;
      }
    });
    this.script = [];
  ...
  populateScriptArray() {
    ...
    for (var key in this.sites) {
      if (this.sites.hasOwnProperty(key)) {
        var siteOptions = this.sites[key];
  ...
  /**
   * Overwrite the database import script to ensure that multisite databases
   * are only included once.
   */
  addScriptImportDatabase(siteOptions) {
    if (siteOptions.options.database) {
      if (!this.importedDatabases.includes(siteOptions.options.database)) {
        var databaseImportBase = '';
        if (siteOptions.options.databaseGzipped) {
          databaseImportBase = 'gunzip -c';
        }
        else if (siteOptions.options.databaseBzipped) {
          databaseImportBase = 'bunzip2 -c';
        }
        else {
          databaseImportBase = 'cat';
        }
        this.script.push(`${databaseImportBase} $ASSET_DIR/${siteOptions.options.database} | $(mysql -u ${constants.DATABASE_USER} --password=${constants.DATABASE_PASSWORD} ${siteOptions.databaseName});`);
        this.script.push(`rm $ASSET_DIR/${siteOptions.options.database};`);
        this.importedDatabases.push(siteOptions.options.database);
      }
    }
  }
  ...
  addScriptDatabaseUpdates(siteOptions) {
    this.script.push(`drush --root=/var/www/html/${siteOptions.options.siteFolder} updb`);
  }

  addScriptRevertFeatures(siteOptions) {
    this.script.push(`drush --root=/var/www/html/${siteOptions.options.siteFolder} fra`);
  }
  ...
  addScriptClearCaches(siteOptions) {
    if (siteOptions.options.drupalVersion >= 8) {
      this.script.push(`drush --root=/var/www/html/${siteOptions.options.siteFolder} cache-rebuild`);
    }
    else {
      this.script.push(`drush --root=/var/www/html/${siteOptions.options.siteFolder} cache-clear all`);
    }
  }

  drupalVersionSupported(siteOptions) {
    const v = siteOptions.options.drupalVersion;
