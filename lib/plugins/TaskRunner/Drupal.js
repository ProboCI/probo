...
class Drupal extends LAMPApp {
  ...
  addScriptSetupDrush() {
    if (this.options.drupalVersion === 7) {
      this.script.push('ln -s /usr/local/src/drush8/vendor/bin/drush /usr/local/bin/drush');
    }
    else if (this.options.drupalVersion === 8) {
      this.script.push('ln -s /usr/local/src/drush9/vendor/bin/drush /usr/local/bin/drush');
    }
    else if (this.options.drupalVersion === 9) {
      this.script.push('ln -s /usr/local/src/drush-launcher/drush /usr/local/bin/drush');
    }
  }
  ...
  addScriptRunInstall() {
    ...
    this.script.push(`drush -y site-install --root=/var/www/html ${this.options.profileName} ${installArgs}`);
  }
  ...
  addScriptRunMakeFile() {
    ...
    this.script.push('cd $SRC_DIR ; drush -y make ' + this.options.makeFile + ' /var/www/html ' + makeArgs);
