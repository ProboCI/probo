...
module.exports = class Drupal extends require('./Script') {
  ...
  addScriptAppendSettingsPHPSettings() {
    ...
    if (options.settingsRequireFile) {
      var command = 'echo "require_once(\'' + this.options.settingsRequireFile + '\');" >> /var/www/html/sites/' + this.options.siteFolder + '/settings.php';
      this.script = this.script.concat([command]);
    }
  }
