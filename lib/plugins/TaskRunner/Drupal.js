...
class Drupal extends LAMPApp {
   ...
   *   @param {object} [options.generateContent] - An object describing content to create, in the format {'content_type': 5, 'content_type2': 5,} 
   *      (where the number is how many nodes to create of each kind).
   *   @param {object} [options.generateUsers] - An object describing users to create, in the format {'role': count, 'role2': count, 'role,role2': count,}.
   *      All users have the password 'strongpassword'. The third item in the object adds users with both role and role1.
   *   @augments LAMPApp
   */
  ...
  populateScriptArray() {
    ...
    if (this.options.generateContent || this.options.generateUsers) {
      this.addScriptDevelGenerate();
      if (this.options.generateContent) {
        this.addScriptDevelGenerateContent(this.options.generateContent);
      }
      if (this.options.generateUsers) {
        this.addScriptDevelGenerateUsers(this.options.generateUsers);
      }
    }
  ...
  addScriptDevelGenerate() {
    this.script.push('cd $SRC_DIR ; drush en devel -y; drush en devel_generate -y;')
  }

  addScriptDevelGenerateContent(types) {
    for (var content_type in types) {
      if (types.hasOwnProperty(content_type)) {
        this.script.push(`cd $SRC_DIR ; drush generate-content ${types[content_type]} --types=${content_type}`);
      }
    }
  }

  addScriptDevelGenerateUsers(roles) {
    for (var role_name in roles) {
      if (roles.hasOwnProperty(role_name)) {
        this.script.push(`cd $SRC_DIR ; drush generate-users ${roles[role_name]} --roles=${role_name} --pass=strongpassword`);
      }
    }
  }
