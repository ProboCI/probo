...
class Drupal extends LAMPApp {
  ...
  constructor(container, options) {
    ...
    this.options.databaseName = options.databaseName || 'drupal';
  ...
  addScriptDatabaseUpdates() {
    this.script = this.script.concat(['drush -y --root=/var/www/html updb']);
  }

  addScriptRevertFeatures() {
    this.script = this.script.concat(['drush -y --root=/var/www/html fra']);
  }
  ...
  addScriptClearCaches() {
    ...
    if (this.options.drupalVersion >= 8) {
      this.script = this.script.concat(['drush -y --root=/var/www/html cr']);
    } else {
      this.script = this.script.concat(['drush -y --root=/var/www/html cc all']);
    }
  }
