...
class Drupal extends LAMPApp {
  ...
  constructor(container, options) {
    ...
    this.redis = this.options.redis || false;
    this.composer = this.options.composer || false;
    this.databaseName = this.options.databaseName || 'drupal';
    this.configImport = this.options.configImport || false;
  ...
  addScriptAppendSettingsPHPSettings() {
    if (this.options.drupalVersion === 7) {
      this.addD7PHPSettings(this.redis);
    } else if (this.options.drupalVersion === 8) {
      this.addD8PHPSettings(this.redis);
    } else if (this.options.drupalVersion === 9) {
      this.addD9PHPSettings(this.redis);
    } else if (this.options.drupalVersion === 10) {
      this.addD10PHPSettings(this.redis);
    } else if (this.options.drupalVersion === 11) {
      this.addD11PHPSettings(this.redis);
    }
    this.appendCustomSettings();
  }

  addD7PHPSettings(redis) {
    if (redis) { }
