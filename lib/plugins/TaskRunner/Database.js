...
module.exports = class Database extends require('./Script') {
  ...
  constructor(container, options) {
    ...
    if (daemon === 'mysql') {
      ...
      script.push('apt install -y postgresql-' + version + ' postgresql-' + version + '-postgis-3');
      script.push('update-rc.d postgresql enable');
      script.push('service postgresql start');
      script.push('su - postgres -c "createuser -d -l -r -s root"');
      script.push('su - postgres -c "createdb --owner=root probo"');
      script.push('psql --username=root --dbname=probo -c "ALTER USER root WITH PASSWORD \'strongpassword\';"');
      ...
      script.push('curl https://packages.microsoft.com/keys/microsoft.asc | sudo tee /etc/apt/trusted.gpg.d/microsoft.asc');
      script.push('curl https://packages.microsoft.com/config/ubuntu/18.04/prod.list | tee /etc/apt/sources.list.d/mssql-release.list');
      script.push('apt update');
      script.push('ACCEPT_EULA=Y apt install -y mssql-tools18 unixodbc-dev');
      // Only configure PHP if PHP is set and .NET is not. If they have both .NET and PHP configured
      // then we should not configure since PHP and .NET are not in the same base container.
      if (options.php && !options.dotnet) {
        script.push('apt install -y msodbcsql18 php-pear');
        script.push('pecl install sqlsrv');
        script.push('pecl install pdo_sqlsrv');
        script.push('printf "; priority=20\nextension=sqlsrv.so\n" > /etc/php/' + options.php + '/mods-available/sqlsrv.ini');
        script.push('printf "; priority=30\nextension=pdo_sqlsrv.so\n" > /etc/php/' + options.php + '/mods-available/pdo_sqlsrv.ini');
        script.push('phpenmod sqlsrv pdo_sqlsrv');
      }
    } else {
      script.push('echo "Invalid database. You can only use mariadb, mysql, mssql or postgresql.');
      script.push('exit 1;');
    }
