...
module.exports = class BitbucketDownloader extends require('./Script') {
  ...
  setBitbucketScript(){
    ...
    var url = 'https://' + `bitbucket.org/${owner}/${repo/get/A0B1C2D.tar.gz

    var script = [
      'mkdir -p $SRC_DIR',
      'cd $SRC_DIR',
      `wget -q -O - --header 'Authorization:${token}' "${params.url}" | tar xzf - `
    ]
  ...
  getToken(done) {
    ...
    var url = 'http://' + `${config.server.host}:${config.server.port}/auth_lookup`;
    var auth = {
      url: url,
      qs: {
        type: 'bitbucket',
