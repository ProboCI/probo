...
class StaticApp extends Script {
  ...
  addScriptSymlinks() {
    this.script = this.script.concat([
      ...
      'fi',
      'if [ -a "$SRC_DIR/build" ] ; then',
      '  echo \'buildDirectory \'build\' found within the root of the codebase, creating symlink.\'',
      '  ln -s $SRC_DIR/build  /var/www/html',
      'fi',
      'if [ -a "$SRC_DIR/public" ] ; then',
      '  echo \'buildDirectory \'public\' found within the root of the codebase, creating symlink.\'',
      '  ln -s $SRC_DIR/public  /var/www/html',
      'fi',
      'if [ -a "$SRC_DIR/_site" ] ; then',
      '  echo \'buildDirectory \'_site\' found within the root of the codebase, creating symlink.\'',
      '  ln -s $SRC_DIR/_site  /var/www/html',
      'fi',
    ]);
  }
