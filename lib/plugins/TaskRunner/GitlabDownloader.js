...
module.exports = class GitlabDownloader extends require('./Script') {
  ...
  setGitlabScript() {
    ...
    var script = [
      'mkdir -p $SRC_DIR',
      'cd $SRC_DIR',
      `curl --header "Authorization: Bearer ${params.auth_token}" https://gitlab.com/api/v4/projects/${params.repo_slug}/repository/archive?sha=${params.ref} | tar xzf - --strip-components=1`,
    ];
 
    this.log.info({script: script}, ' ONG THE SCRIPT');
