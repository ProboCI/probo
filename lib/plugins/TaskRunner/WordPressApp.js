...
class WordPressApp extends LAMPApp {
  ...
  /**
   * This is the config we use if there is no wp-config.php in the repo.
   */
...
require_once(ABSPATH . 'wp-settings.php');`;
  ...
  /**
   * If there is a wp-config.php, we need to add some defines before the existing config, but before wp-settings is called. Please note that there's no need to add a <?php tag since it should already be there.
   */
  wordpressConfigOverride() {
    return `define('DB_NAME', '${this.databaseName}'); \
define('DB_USER', '${constants.DATABASE_USER}'); \
define('DB_PASSWORD', '${constants.DATABASE_PASSWORD}'); \
define('DB_HOST', 'localhost'); \
`;
