...
class WordPressApp extends LAMPApp {
  ...
  addScriptUpdatePlugins() {
    this.script.push('cd $SRC_DIR ; wp plugin update --allow-root');
  }

  addScriptFlushCaches() {
    this.script.push('cd $SRC_DIR ; wp cache flush --allow-root');
  }
  ...
  replaceOption(updated, option) {
    var command = `cd $SRC_DIR/${this.subDirectory} ; wp option update ${option} ${updated} --allow-root`;
    return command;
  }
  ...
  replaceTextDb(orig, updated) {
    var command = `cd $SRC_DIR/${this.subDirectory} ; wp search-replace '${orig}' '${updated}' --skip-columns=guid --allow-root`;
    return command;
  }
}
