...
describe('StepFactory', function() {
  it('should instantiate steps based on provided step configuration', function() {
    ...
    var build = new Build();
    var stepFactory = new StepFactory({steps, container, build});
    var stepConf = [
      {
        name: 'Some Step',
        plugin: 'TestStep',
    ...
    result[0].constructor.should.equal(steps.TestStep);
  ...
  it('should alias plugin and type to look up the plugin configuraiton', function() {
    var container = new MockContainer();
    var stepFactory = new StepFactory({steps, container});
    var stepConf = [
      {
        name: 'Step 1',
        type: 'TestStep',
  ...
  it('should handle nested StepList definitions', function() {
    ...
    var stepFactory = new StepFactory({steps, container, build});
    var result = stepFactory.createStepsFromConfig(stepConf);
    result[0].constructor.should.equal(steps.StepList);
    result[0].steps[0].constructor.should.equal(steps.Shell);
    result[0].steps[0].options.command.should.containEql('Disco Stu');
    result[0].steps[1].steps[0].constructor.should.equal(steps.Script);
